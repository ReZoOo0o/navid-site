...
describe('rendered pages', () => {
  ...
  it('noscript safety: reveal hiding is scoped to .js (M6)', () => {
    // The flag used to be an inline <script> in Base.astro. The origin serves
    // `script-src 'self'`, which refuses inline script, so the deployed page
    // set no flag at all and the M6 guarantee held only by accident (nothing
    // hid .reveal either, because that script was refused too). It is a file
    // now, so this asserts the whole mechanism rather than one spelling of it:
    // both pages load it, and the file that ships actually sets the class.
    for (const page of [en(), fa()]) expect(page).toMatch(/<script[^>]+src="\/js-flag\.js"/);
    expect(existsSync('dist/js-flag.js')).toBe(true);
    expect(readFileSync('dist/js-flag.js', 'utf8')).toMatch(/documentElement\.classList\.add\(['"]js['"]\)/);
    // and no executable inline script survives, in either locale — the one
    // inline block left is the JSON-LD data island, which is not executed.
    for (const page of [en(), fa()]) {
      const inline = [...page.matchAll(/<script(?![^>]*\ssrc=)([^>]*)>/g)]
